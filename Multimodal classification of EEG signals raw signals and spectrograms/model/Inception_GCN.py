...
class Inceptionv1(nn.Module):
    ...
    def forward(self, x):
        ...
        # 将这四个模块沿着通道方向进行拼接
        output = torch.cat((b1, b2, b3, b4), dim=3)
...
class MyGraphConvolution(nn.Module):
    ...
    def kernel(self, adjacency):
        ...
        D = D*(torch.eye(self.channels_num).to(device))
        L = D - adjacency
        # 计算特征值与特征向量
        e_vales, e_vets = torch.linalg.eig(L)
        # 计算模型需要的参数
...
class latent_correlation_layer(nn.Module):
    def __init__(self, num_features):
        ...
        self.weight_key = nn.Parameter(torch.zeros(size=(num_features, 3)))
        nn.init.xavier_uniform_(self.weight_key.data, gain=1.414)
        self.weight_query = nn.Parameter(torch.zeros(size=(num_features, 3)))
...
class IGCN(nn.Module):
    def __init__(self, num_features=128, leaky_rate=0.2, kernel=5, channel_num=None):
        ...
        self.Iconv = Inceptionv1(1,1,1,1,1,1,1)
        ...
        self.F = nn.Flatten()
        self.dropout = nn.Dropout(p=0.4)
        self.latent_correlation_layer1 = latent_correlation_layer(num_features=self.num_features*4)
