...
def gen_edges_wpli(x, fs=128, weighted=True):
    """
...
def gen_my_edge(x):
    ...
    :return: edge_index: (2, C * C - C)
    """
    edges = np.load('./utils/edges64.npy')
    ...
    # 转换成无向图
    edge_index = np.concatenate((edges1, edges), axis=1)
    weight = np.ones((edge_index.shape[1], 1))
...
def dirictedge(x):
    ...
    :return: edge_index: (2, C * C - C)
    """
    edges = np.load('../gnn-eeg-master/utils/edges.npy')
    edges = edges.transpose(1, 0)
    # edge_index = torch.tensor(edge_index)
    weight = np.ones((edges.shape[1], 1))
